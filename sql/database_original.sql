...
create table if not exists customer (
    ...
    user_id integer not null references useraccount on delete cascade,
...
alter table staffmember owner to postgres;
create table if not exists address (
    address_id serial primary key,
    user_id integer not null references useraccount on delete cascade,
    address_type varchar(20) not null constraint address_address_type_check check (
...
create table if not exists creditcard (
    ...
    payment_address_id integer not null references address,
    is_default boolean default false
);
...
create table if not exists product (
    ...
    category_id integer not null references productcategory,
...
create table if not exists customerorder (
    ...
    status varchar(20) not null constraint customerorder_status_check check (
        (status)::text = ANY (
            (
    ...
    card_id integer not null references creditcard,
    total_amount numeric(10, 2) not null
);
alter table customerorder owner to postgres;
create index if not exists idx_order_customer on customerorder (customer_id);
create index if not exists idx_order_status on customerorder (status);
create index if not exists idx_order_date on customerorder (order_date);
create table if not exists deliveryplan (
    delivery_id serial primary key,
    order_id integer not null references customerorder,
    delivery_type varchar(20) not null constraint deliveryplan_delivery_type_check check (
    ...
    ship_date date,
    delivery_date date,
    delivery_address_id integer not null references address
);
alter table deliveryplan owner to postgres;
...
alter table supplierproduct owner to postgres;
create or replace function check_warehouse_capacity() returns trigger language plpgsql as $$
DECLARE total_used_capacity DECIMAL(10, 2);
warehouse_max_capacity DECIMAL(10, 2);
BEGIN -- Get the warehouse capacity
SELECT capacity INTO warehouse_max_capacity
FROM Warehouse
WHERE warehouse_id = NEW.warehouse_id;
-- Calculate total used capacity (including the new stock)
SELECT COALESCE(SUM(s.quantity * s.unit_size), 0) INTO total_used_capacity
FROM Stock s
...
IF total_used_capacity > warehouse_max_capacity THEN RAISE EXCEPTION 'Warehouse capacity exceeded. Available: %, Required: %',
...
create trigger enforce_warehouse_capacity before
insert
    or
update on stock for each row execute procedure check_warehouse_capacity();
create or replace function update_product_timestamp() returns trigger language plpgsql as $$ BEGIN NEW.last_updated := CURRENT_TIMESTAMP;
RETURN NEW;
END;
$$;
alter function update_product_timestamp() owner to postgres;
create trigger product_timestamp_update before
update on product for each row execute procedure update_product_timestamp();
create or replace function check_product_availability() returns trigger language plpgsql as $$
...
IF available_quantity < NEW.quantity THEN RAISE EXCEPTION 'Insufficient stock for product "%". Available: %, Requested: %',
product_name,
available_quantity,
NEW.quantity;
END IF;
...
create trigger enforce_product_availability before
insert on orderitem for each row execute procedure check_product_availability();
create or replace function update_customer_balance() returns trigger language plpgsql as $$ BEGIN -- Add the order total to the customer's balance
UPDATE Customer
...
after
insert on customerorder for each row execute procedure update_customer_balance();
create or replace function update_stock_after_order() returns trigger language plpgsql as $$
...
BEGIN remaining_quantity := NEW.quantity;
-- Iterate through warehouses to deduct stock
OPEN warehouse_cursor;
LOOP FETCH warehouse_cursor INTO warehouse_record;
EXIT
WHEN NOT FOUND
OR remaining_quantity <= 0;
IF warehouse_record.quantity >= remaining_quantity THEN -- Enough stock in this warehouse
UPDATE Stock
...
after
insert on orderitem for each row execute procedure update_stock_after_order();